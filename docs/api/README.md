...
| `interaction-service-openapi.json` | interaction-service (`:8084`) | http://localhost:8084/interaction/swagger-ui | `/interaction/v3/api-docs/interaction-v1` |
...
```bash
INTERACTION_SERVICE_PORT=18084 ./scripts/dump-openapi.sh interaction
```
...
- 손으로 쓴 endpoint 표(`*_260724_01.md`) 대신 이 스냅샷 + Swagger UI를 규격의 소스로 쓴다.
  도메인 규칙(멱등성, soft-delete, flush 주기 등)만 별도 문서에 남긴다.
